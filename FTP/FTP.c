...
int handle_ls(const char* arg, int client_socket, int data_socket){
  ...
  while(bytes_rcvd < byte_count && (status = recv(data_socket, buffer, sizeof(buffer), 0)) > 0){
...
int handle_cwd(const char* arg, int client_socket){
  ...
  if(chdir(arg) < 0){
    sprintf(response, "Can't change directory to: \"%s\": No such file or directory", arg);
...
int handle_user(const char* arg, char* user_name, int client_socket){
  for(size_t i = 0; i < NUM_USERS + 1; ++i)
    if(i == NUM_USERS)
      ...
      return send_response("331", "User name okay, need password", client_socket);
    }
  return -1;
}
