...
int main(int argc, char* argv[]) {
  int server_socket, client_socket, client_count = 0, port = PORT_NO,
                                    bytes_rcvd = 0, bytes_sent = 0,
                                    data_sock = INT_MIN;
  char out_buffer[BUF], in_buffer[BUF], user_name[BUF], cwd[BUF],
      response[BUF2];
  ...
  while ((client_socket =
    ...
    if (fork() == 0) {
      if ((bytes_sent = send_response("220", welcome_message, client_socket)) <=
          0) {
